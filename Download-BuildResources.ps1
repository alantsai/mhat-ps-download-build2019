...
### Variables ###
$api = 'https://api.mybuild.techcommunity.microsoft.com/api/session/all'
...
function BuildDownloadDirectory($path) {
    $repeatLoop = $true;
    while ($repeatLoop) {
        $repeatLoop = $false
...
function DownloadSession($sessionObject, $sessionSearchCount, $directory) {
    if (($sessionObject.slideDeck.Length -ne 0) -or ($sessionObject.downloadVideoLink.Length -ne 0)) {
        ...
        if (!(test-path $dataFile)) {
            ...
            $metaData += "Session ID: " + $sessionObject.sessionId;
            $metaData += "`r`nSession Code: " + $sessionObject.sessionCode;
...
### Main ###
BuildDownloadDirectory $directory;
